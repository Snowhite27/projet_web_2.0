                ...
                <div class="p-6 bg-white border-b border-gray-200 flex flex-col justify-center items-center">
                    <form class="flex justify-start items-center w-full flex-col" action="{{ route('admin.articles.edit.submit', $article->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="w-full">
                            ...
                            <x-input id="name" class="block mt-1 w-full" type="text" name="name" value="{{ $article->name }}" required />
                        </div>

                        {{-- Description --}}
                        <div class="mt-4 w-full">
                            <x-label for="name" :value="__('Description')" />
                            <textarea name="description" rows="10" class="block mt-1 w-full rounded" required>{{ $article->description }}</textarea>
                        </div>
